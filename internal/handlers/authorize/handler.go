...
func (h *Handler) ServerHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		if errors.Is(err, custom_errors.ErrNotFound) {
			h.log.Error("error authorize", "error:", err)
			apiError, _ := json.Marshal(custom_errors.ApiError{Message: "cannot find user"})
			w.WriteHeader(http.StatusUnauthorized)
			w.Write(apiError)
			return
		}

		if errors.Is(err, custom_errors.ErrIsTruePassword) {
			h.log.Error("error authorize", "error:", err)
			apiError, _ := json.Marshal(custom_errors.ApiError{Message: "incorrect password"})
		...
		h.log.Error("error authorize", "error:", err)
		apiError, _ := json.Marshal(custom_errors.ApiError{Message: "cannot authorize user"})
