...
- Основное потребители памяти:
    - runtime.allocm (30.64%) - выделение памяти для M (машин)
    - zap.newCounters (22.94%) - инициализация логгера
    - validator.map.init.7 (15.69%) - инициализация валидатора
    - runtime.procresize (15.43%) - настройка процессоров
    - runtime.acquireSudog (15.29%) - работа с каналами

### Результаты оптимизации

##### Потребление памяти ДО оптимизации:
- Общее потребление: ~3.35MB
- Основные потребители: runtime.allocm, zap.newCounters, validator

#### Потребление памяти ПОСЛЕ оптимизации:
- Общее потребление: ~3.27MB
- Уменьшение на ~2.4%

#### Ключевые улучшения:
1. **Логгер**: Отключены функции caller и stacktrace для экономии памяти
2. **HTTP-сервер**: Добавлены таймауты и ограничения буферов
3. **Процессор заказов**: Оптимизирован размер каналов
